...
exports.encrypto_password = (password) => {
    return new Promise((res) => {
        crypto.pbkdf2(password, config.crypto.salt, config.salt.time, config.salt.byte, config.salt.hash, (err, key) => {
            res(key.toString(config.salt.type));
        });
    });
}
